...
import pokecube.core.events.handlers.SpawnHandler.ForbiddenEntry;
...
{
    private static final Map<ChunkCoordinate, ForbiddenEntry> forbiddenSpawningCoords;
    static
    {
        try
        {
            ...
            forbiddenSpawningCoords = (Map<ChunkCoordinate, ForbiddenEntry>) f.get(null);
        }
        catch (Exception e)
        {
        ...
        if (event.type == Type.DISPLAY_UPDATE)
        {
            ...
            for (ChunkCoordinate c : forbiddenSpawningCoords.keySet())
            {
                try
                {
                    ...
                    // }
                    // int size = forbiddenSpawningCoords.get(c).range;
                    //
                    // MapPolygon repelArea = new MapPolygon(c.east(size +
                    // 1).north(size),
                    // c.west(size).north(size).up(size),
                    // c.west(size).south(size + 1),
                    ...
                    //
                    // overlay = new PolygonOverlay(getModId(), "repel_" + c.dim
                    // + '_' + c.getX() + '_' + c.getZ(), c.dim,
                    // shapeProperties, repelArea);
                    // overlay.setOverlayGroupName("Repels").setTitle("Repelled
                    // Area");
